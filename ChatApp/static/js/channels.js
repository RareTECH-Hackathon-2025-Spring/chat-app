...
// // 3. <li> を組み立てるヘルパー関数 
// // function createLi(teamId, channelId, channelName, channelDescription) {
// //   const li = document.createElement('li');

// //   // チャンネルへのリンクを作成
// //   const a = document.createElement('a');
// //   a.textContent = channelName; // リンクテキスト
// //   a.href        = `/messages/${teamId}/${channelId}/`;// 遷移先URL
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelectorAll('.edit-btn').forEach(button => {
      button.addEventListener('click', function () {
          ...
          const description = this.dataset.channelDescription;

          document.getElementById('editChannelName').value = name;
          document.getElementById('editChannelDescription').value = description;
          document.getElementById('editForm').action = `/channels/${teamId}/update/${channelId}`;
  ...
  document.querySelectorAll('.delete-btn').forEach(button => {
      button.addEventListener('click', function () {
          const channelId = this.dataset.channelId;
          document.getElementById('confirmDeleteLink').href = `/channels/${teamId}/delete/${channelId}`;
